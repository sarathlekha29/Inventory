...
class OrderControllerTest2 {
	...
	public void testCreateOrder() throws Exception {
		...
		Order order = new Order(LocalDate.of(2022, 6, 30), LocalDate.of(2022, 7, 30), 1200f, customer, products);

		Mockito.when(orderService.createOrder(Mockito.any(Order.class))).thenReturn(order);
		MvcResult mvcResult = this.mockMvc
				.perform(MockMvcRequestBuilders.post(URI, 30L).accept(MediaType.APPLICATION_JSON)
